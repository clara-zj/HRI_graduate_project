...
class CaboMotionDiablo:
    ...
    def on_follow(self, turn_left, forward):
        self.send_msg(left_speed=float(turn_left)*0.1, 
                      forward_speed=float(forward)*WALKING_SPEED)
        # if turn_left != 0:
        #     self.send_msg(left_speed=float(turn_left))
        #     time.sleep(FOLLOW_GAP)
        # if forward!= 0:
        #     self.send_msg(forward_speed=float(forward))
        #     time.sleep(FOLLOW_GAP)
        
        self.log(f'Post CMD --- Turn left: {turn_left}, forward: {forward}')
